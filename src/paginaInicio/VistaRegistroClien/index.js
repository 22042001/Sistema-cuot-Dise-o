...
const VistaRegistroClien = () => {
    ...
    const handleMoreInfo = (cliente) => {
        navigate('/informacion-cliente', { state: { cliente } });
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {clientes.map((cliente, index) => (
                    <Card
                        ...
                        carnet={cliente.carnet} // Mostrar el Carnet de Identidad
                        nombre={cliente.nombre}
                        onMoreInfo={() => handleMoreInfo(cliente)}
                    />
                ))}
            </div>
        </div>
